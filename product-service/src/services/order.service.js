...
class OrderService {
  /**
   * Creates a new order with complex transaction management
   * Handles product variants, stock validation, inventory updates, and tax calculations
   * Uses database transactions to ensure data consistency across multiple operations
   */
  static async createOrder(orderData) {
    ...
    try {
      ...
      // Process each order item with validation and stock management
      for (const item of items) {
        ...
        if (variants.length > 0) {
          // Handle products with variants - variant selection is mandatory
          if (!item.variant_id) {
          ...
          // Update inventory levels for both variant and main product
          ...
          // Handle products without variants - use main product stock and pricing
      ...
      // Calculate final totals (allow override from request if provided)
